  ...
  <div className='column is-12'>
    ...
    <table className='table is-bordered is-fullwidth smart-report-table'>
      ...
      <tbody>
       ...
       <tr>
          <td width={150}>{hub.hub_name}</td>
         {hub&&hub?.hubs_data?.map((data:any)=>(
          <td width={20}>{data!==0 &&(<span className='is-size-7'>{data}</span>)} {data===0 &&(<span className='has-text-danger is-size-4'>+</span>)}</td>
         ))}
        </tr>
         ))}
